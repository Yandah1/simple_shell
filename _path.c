...
void check_for_path(shell_vars_t *shell_vars)
{
	...
	if (check_for_dir(shell_vars->arguments[0]))
	...
	else
	{
		...
		if (path != NULL)
		{
			...
			path_tokens = _tokenize(path_dup, ":");
			for (i = 0; path_tokens && path_tokens[i]; i++, free(check))
			{
				check = _strcat(path_tokens[i], shell_vars->arguments[0]);
				if (stat(check, &buf) == 0)
				{
...
int execute_cwd(shell_vars_t *shell_vars)
{
	...
	if (stat(shell_vars->arguments[0], &buf) == 0)
	{
		if (access(shell_vars->arguments[0], X_OK) == 0)
		{
			child_pid = fork();
			if (child_pid == -1)
				print_error(shell_vars, NULL);
			if (child_pid == 0)
			{
				if (execve(shell_vars->arguments[0], shell_vars->arguments,
					shell_vars->env) == -1)
